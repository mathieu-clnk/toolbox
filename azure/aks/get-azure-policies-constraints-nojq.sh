...
get_default() {
  ...
  do
    ...
    kubectl get ${kind} -o custom-columns="kind":.kind,"name":.metadata.name,"enforcement":.spec.enforcementAction,"violations":.status.totalViolation,"assignment-id":.metadata.annotations.azure-policy-assignment-id,"definition-id":.metadata.annotations.azure-policy-definition-id,"setdefinition-id":.metadata.annotations.azure-policy-setdefinition-id --no-headers
  done
}
